...
public:
    ...
    OutputImage(const std::string &filename) : out(std::ofstream(filename)) {}

    virtual void write_image(uint8_t *buffer, int image_width, int image_height) const = 0;
...
public:
    ...
    void write_image(uint8_t *buffer, int image_width, int image_height) const override
    {
        out << (binary ? "P6" : "P3") << "\n"
        ...
        if (binary)
        {
            ...
            {
                out << buffer[i + 0] << buffer[i + 1] << buffer[i + 2];
            }
        }
        else
        {
            for (int i = 0; i < image_width * image_height * 3; i += 3)
            {
                out << int(buffer[i + 0]) << ' ' << int(buffer[i + 1]) << ' ' << int(buffer[i + 2]) << '\n';
            }
        }
...
public:
    Bitmap(const std::string &filename)
        : OutputImage(filename) { out = std::ofstream(filename, std::ios::binary); }
    ...
    void write_image(uint8_t *buffer, int image_width, int image_height) const override
    {
        ...
        for (int j = image_height - 1; j >= 0; j--)
        {
            ...
            {
                uint8_t *rgb = buffer + (image_width * j + i) * 3;
